...
class Project extends Component {
  ...
  showConfirmDeleteProyect(recoder) {
    confirm({
      ...
      content:
        "Este proyecto sera cancelado y quitado de la vista de los usuarios",
      okText: "Eliminar",
      okType: "danger",
      cancelText: "No",
      onOk: () => {
        this.deleteProyect(recoder.idProject);
      },
    });
  }
  ...
  createProyect = async () => {
    if (this.validate()) {
      ...
      if (response.result === "created") {
        success({
          content:
            "se ha creado un proyecto y se han agregado los emprendores con exito",
        });
        const notification = {
          to: this.state.value,
          message: `El asesor ${localStorage.getItem(
            "username"
          )} te ha invitado a trabajar`,
  ...
  validate() {
    if (
      this.state.nameProyect != "" &&
      this.state.tagsmethodologies.length > 0 &&
      this.state.nameAsesor != "" &&
  ...
  handleClose = (removedTag) => {
    const tagsmethodologies = this.state.tagsmethodologies.filter(
      (tag) => tag !== removedTag
    );
                  ...
                  <Form.Item>
                    ...
                    >
                      {this.state.Entre.map((Entrepreneurs) => {
                        return (
                          <TreeNode
                            value={Entrepreneurs.name}
                            title={Entrepreneurs.name}
                          />
                        );
                      })}
                    </TreeSelect>
