...
class AssigmentAdviser extends React.Component {
    ...
    render(){
        return(
        <Modal  width="40%" title="Asignar asesores a una fase metodologica" visible={this.props.showModalAssigment}
                ...
                <Col span={12}>
                    <Form>
                        <Form.Item>
                            ...
                            <Select className="Select_Assigment" onChange={this.onChangeAdvisers}>
                                 {this.state.advisers.map(e => (<Option key={e.idUsers}>{e.name}</Option>))}
                            </Select>
                        </Form.Item>
                        <Form.Item>
                            <label>Selecione una fase</label>
                            <Select  className="Select_Assigment" onChange={this.onChangePhases}>
                                {this.props.phases.map(e => (<Option key={e.phase}>{e.phase}</Option>))}
                            </Select>
                        </Form.Item>
                        <Form.Item>
                            <Button  type="primary" className="Select_Assigment" onClick={this.createAssignment}>Asignar fase</Button>
                        </Form.Item>
                    </Form>
                </Col>
            </Row>
