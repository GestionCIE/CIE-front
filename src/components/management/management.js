...
class management extends Component {
  ...
  validateDate(start, end) {
    ...
    console.log(w_end);
    if (
      w_start.year >= now.year &&
      w_start.month >= now.month &&
      w_start.day >= now.day
    ) {
      console.log("primer cond");
      if (
        w_start.year <= w_end.year &&
        w_start.month <= w_end.month &&
        w_start.day <= w_end.day
      ) {
  ...
  async getAllProjects() {
    const response = await http.get(
      `project/getProjects?type=${localStorage.getItem(
        "role"
  ...
  getParticipants = async (id) => {
    ...
    for (let i = 0; i < response.result.entrepreneurs.length; i++) {
      options.push(
        <Option value={response.result.entrepreneurs[i].name} key={i + 1}>
          {response.result.entrepreneurs[i].name}
        </Option>
      );
    }
    console.log(options);
  ...
  getActivities = async (id, phase) => {
    const response = await http.get(
      `project/getActivityByProjectAndPhase?id=${id}&phase=${phase}`
    );
  ...
  validate(assings) {
    if (
      this.state.nameActivity != "" &&
      assings != "" &&
      this.state.stateActivity[0] != "" &&
      this.state.phaseSelect != "" &&
      this.state.idProject != -1 &&
  ...
  async getActivityById() {
    const response = await http.get(
      `project/getActivity?id=${this.state.idActivityEdit}`
    );
  ...
  getAssignment = async () => {
    const response = await http.get(
      `project/getAssignment?id=${this.state.idProject}&phase=${this.state.phaseSelect}`
    );
    console.log("adviser assigned ", response.result.nameAssigned);
    this.setState({
      nameAssigned: response.result.nameAssigned,
  ...
  showModalAssigment = () => {
    this.setState({ showModalAssigment: true });
  };
    ...
    const originColumns = [
      {
        ...
        key: "id",
        render: (text, recoder) => (
          <Checkbox onClick={() => this.getIdActivity(recoder)} />
        ),
      },
      {
        title: "Actividad",
...

      {
        ...
        key: "responsables",
        render: (text, recoder) => (
          <AvatarComponent profiles={recoder.profile} />
        ),
      },
      {
            ...
            <Col span={20}>
              ...
              >
                <Form className="Form_Modal">
                  ...
                  <Form.Item>
                    <label>Añadir semana de la actividad</label>
                    <br />
                    <RangePicker
                      defaultValue={moment(new RangePicker())}
                  ...
                  <Form.Item>
                    <Button
                      onClick={() => {
                        this.state.titleModal == "Crear Actividad"
                          ? this.createActivity()
                          : this.EditActivity();
                      }}
              ...
              <AssigmentAdviser
                showModalAssigment={this.state.showModalAssigment}
              ...
              {this.state.showComponent ? (
                <DetailActivity
                  ...
                  detailtActivity={this.state.detailtActivity}
                  reloadActivities={this.callgetActivities}
                  idProject={this.state.idProject}
                  phase={this.state.phaseSelect}
              ...
              <Row>
                <Col span={10}>
                  <div
                    ...
                    style={{ display: this.state.visibleImg }}
                  >
                    <img className="Img_Management" src={img_management} />
                  </div>
                  <div
                    className="Content_Methodologies"
                    style={{ display: this.state.visibleList }}
                  >
                    <List
                      header={<h5>Fases Metodologicas</h5>}
                      bordered
                ...
                <Col span={14}>
                  {this.state.visibleContent ? (
                    <>
                      <Space size={8}>
                        <Button onClick={() => this.setVisibleModal("add")}>
                          Crear actividad
                        </Button>
                        <Button onClick={() => this.setVisibleModal("edit")}>
                          Editar actividad
                        </Button>
