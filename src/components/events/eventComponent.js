...
class EventComponent extends React.Component {
  ...
  showConfirmDeleteEvent(recoder) {
    confirm({
      ...
      content:
        "Este evento sera cancelado y quitado de la vista de los usuarios",
      okText: "Eliminar",
      okType: "danger",
      cancelText: "No",
      onOk: () => {
        this.deleteEvent(recoder.idEvents);
      },
    });
  }
  ...
  createEvent = async () => {
    ...
    if (response.result === "created") {
      const response_statistics = await http.post(
        "event/createEventStatistics",
        { idEvent: response.id }
      );
  ...
  howIsButton() {
    ...
    if (this.state.edit) {
      button = (
        <Button
          stye={{ marginleft: "2%" }}
          className="Form_Event_Button"
          onClick={this.updateEvent}
        >
          {" "}
          <EditOutlined /> Editar Evento
        </Button>
      );
    } else {
      button = (
  ...
  cancelEvent() {
    ...
    if (this.state.edit) {
      button = (
        <Button className="Form_Event_Button" onClick={this.handleCancelEvent}>
          {" "}
            ...
            <Row>
              ...
              <Col>
                <div className="Div_Position">
                  <Button type="primary" onClick={this.setVisibleModal}>
