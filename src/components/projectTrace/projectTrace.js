...
class ProjectTrace extends React.Component {
  ...
  async getAllProjects() {
    const response = await http.get(
      `project/getProjects?type=${localStorage.getItem("role")}`
    );
    this.setState({
  ...
  async getPhases() {
    const response = await http.get(
      `project/phases?id=${this.state.idProject}`
    );
      ...
      <Row>
        ...
        <Col span={24}>
          <Space size={15}>
            ...
            {this.state.otherControls ? (
              <Select
                defaultValue="Seleciona una Fase"
                className="Select_Inputs"
                onChange={this.onChangeGantt}
              >
        ...
        <Col span={24}>
          {this.state.visibleGraph ? (
            ...
            <img className="Img_Char" src={charImg} />
          )}
        </Col>
      </Row>
    );
