...
class ProfesionalProfile extends React.Component{
    ...
    registerData = async () => {
        ...
        if(response.result === 'created'){
            success({content: 'Se Agrego la informacion profesoinal correctamente'});
        }else{
            error({content: 'A ocurrido un error al agregar la información profesional'})
        }
    }
    ...
    render(){
        return(
           ...
           <Row>
               ...
               footer={null} onCancel={this.closeModal} title="Informacion Profesional">
                   <Row>
                       <Col span={24}>
                            <Form>
                                <Form.Item>
                                    <label>Titulo Profesional</label><br/>
                                    <Input className="Form_Item_Input" value={this.state.universityDegrees} name="universityDegrees" 
                                        placeholder="Titulo Profesional" onChange={this.onchangeData}/>
                                </Form.Item>

                                <Form.Item>
                                    <label> Años de Experiencia Profesional</label> <br/>
                                    <InputNumber className="Form_Item_Input"  value={this.state.experience} name="experience"
                                        placeholder="Exp" onChange={this.onChangeExp}/>          
                                </Form.Item>

                                <Form.Item>
                                    <Button type="primary" className="Form_Item_Button" onClick={this.registerData}>Añadir Datos profesionales</Button>
